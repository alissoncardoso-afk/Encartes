...
// ─── Mini Components ──────────────────────────────────────────────────────────
function CircleProgress({ pct, size = 88, stroke = 7, color = ACCENT, label, sub }) {
    ...
    <div style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:0 }}>
      <div style={{ position:"relative", width:size, height:size }}>
        ...
        <div style={{ position:"absolute", inset:0, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center" }}>
          <span style={{ fontSize:size*.21, fontWeight:800, color:"#f1f5f9", fontFamily:"'DM Serif Display',serif", lineHeight:1 }}>{pct}%</span>
        </div>
      </div>
      {label && <div style={{ fontSize:10, color:"#94a3b8", fontWeight:700, letterSpacing:1, textTransform:"uppercase", marginTop:6 }}>{label}</div>}
...
function Tag({ color, children }) {
  return (
    <span style={{ background:color+"22", color, fontSize:9, fontWeight:700, padding:"2px 7px", borderRadius:20, letterSpacing:.5, textTransform:"uppercase", flexShrink:0 }}>
      {children}
    </span>
  );
}
...
function PriorityDot({ p }) {
  const c = p === "alta" ? RED : p === "media" ? ACCENT : GREEN;
...
export default function App() {
  ...
  const weeklyScores = last7.map(date => {
    const hLog = logs[date] || {};
    const tLog = taskLogs[date] || [];
    const dayTasks = tasks.filter(t => t.date === date);
    const hP = habits.length ? habits.filter(h=>hLog[h.id]).length/habits.length : 0;
    const tP = dayTasks.length ? tLog.filter(id=>dayTasks.find(t=>t.id===id)).length/dayTasks.length : 0;
  ...
  const monthDays   = Array.from({length:daysInMonth}, (_,i) => {
    ...
    const hLog = logs[d]||{};
    const tLog = taskLogs[d]||[];
    const dayTasks = tasks.filter(t=>t.date===d);
    const hP = habits.length ? habits.filter(h=>hLog[h.id]).length/habits.length*100 : 0;
    const tP = dayTasks.length ? tLog.filter(id=>dayTasks.find(t=>t.id===id)).length/dayTasks.length*100 : 0;
  ...
  return (
    <div style={S.root}>
      ...
      <header style={S.header}>
        ...
        <div style={S.streakBadge}>🔥 {currentStreak} dias</div>
      </header>
          ...
          {weeklyScores.map(({date,score}) => (
            <div key={date} style={{ flex:1, display:"flex", flexDirection:"column", alignItems:"center", gap:3 }}>
              ...
              <div style={{
                ...
                boxShadow: date===today?`0 0 10px ${ACCENT}55`:"none"
              }} />
      ...
      <div style={S.tabs}>
        {[["hoje","📋 Hoje"],["habitos","🔄 Hábitos"],["relatorio","📊 Mês"]].map(([k,l]) => (
          <button key={k} onClick={()=>setTab(k)} style={{...S.tabBtn,...(tab===k?S.tabActive:{})}}>
            {l}
          </button>
        ))}
      </div>
      ...
      {tab==="hoje" && (
        <div style={S.section}>
          <div style={{...S.secTitle, ...S.row, justifyContent:"space-between"}}>
          ...
          {todayTasks.map(task => {
            ...
            return (
              <div key={task.id} style={{...S.taskRow, opacity:done?.5:1}}>
                <button onClick={()=>toggleTask(task.id)} style={{...S.check,...(done?S.checkDone:{})}}>
                  {done&&"✓"}
                </button>
                <span style={{ flex:1, textDecoration:done?"line-through":"none", color:done?"#64748b":"#e2e8f0", fontSize:13 }}>
                  {task.title}
                </span>
                <PriorityDot p={task.priority} />
                <Tag color={task.priority==="alta"?RED:task.priority==="media"?ACCENT:GREEN}>{task.priority}</Tag>
        ...
        <div style={S.section}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10, marginBottom:20 }}>
            {[
              ...
              <div key={label} style={{ background:"#0f172a", border:`1px solid ${color}33`, borderRadius:14, padding:"14px 14px" }}>
...
// ─── Styles ───────────────────────────────────────────────────────────────────
const styles = {
  root:        { minHeight:"100vh", background:"#020817", color:"#e2e8f0", fontFamily:"'DM Sans','Segoe UI',sans-serif", maxWidth:480, margin:"0 auto", padding:"0 0 40px", position:"relative", overflow:"hidden" },
  ...
  greeting:    { fontSize:21, fontWeight:700, color:"#f1f5f9", fontFamily:"'DM Serif Display',serif", letterSpacing:-.3 },
  dateStr:     { fontSize:11, color:"#64748b", marginTop:4, letterSpacing:.3 },
  streakBadge: { background:"linear-gradient(135deg,#f59e0b22,#f5a62344)", border:"1px solid #f5a62355", color:ACCENT, fontSize:11, fontWeight:700, padding:"6px 12px", borderRadius:20, letterSpacing:.3, flexShrink:0 },
  ...
  tabBtn:      { flex:1, padding:"9px 4px", borderRadius:11, border:"1px solid #1e293b", background:"transparent", color:"#64748b", fontSize:11, fontWeight:600, cursor:"pointer", letterSpacing:.3, transition:"all .2s" },
  tabActive:   { background:`linear-gradient(135deg,#F5A62322,#60A5FA11)`, border:`1px solid ${ACCENT}55`, color:"#f1f5f9", boxShadow:`0 0 14px ${ACCENT}22` },
  ...
  taskRow:     { display:"flex", alignItems:"center", gap:9, background:"rgba(15,23,42,.85)", border:"1px solid #1e293b", borderRadius:11, padding:"11px 12px", marginBottom:7, transition:"opacity .3s" },
  check:       { width:22, height:22, borderRadius:6, border:"1.5px solid #334155", background:"transparent", color:GREEN, cursor:"pointer", fontSize:11, display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0, transition:"all .2s" },
  checkDone:   { background:GREEN+"33", border:`1.5px solid ${GREEN}`, color:GREEN },
  ...
  habitChip:   { padding:"7px 12px", borderRadius:20, fontSize:12, fontWeight:600, cursor:"pointer", transition:"all .25s", letterSpacing:.2 },
